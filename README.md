...
- 蓝紫色：汉字笔划偏旁字符（若成完整的字，正常用于句中时应该用对应的统一字符）
- 橙色边框：收录在Unicode扩展区的汉字（一般不是很常见的字）
- 红紫色：Unicode兼容汉字符（不应使用，应该用对应的统一字符）
- 灰色：未定义编码字符

## 方向及限制

1. 因不同地区、文化圈、系统，不同的历史流行输入法、字体，机构所沿用的信息系统的原因，所谓汉字“规范”、“通行”、“正常”字符到底该依照哪个（统一字、部首字符、兼容字，还是PUA，甚至不用万国码而用一国码？），本项目（至少目前）无法收录和处理完整的这类信息。
...
下例为总合了 中华地区官方中文教育制定的常用字表 后的部分数据
```json
"說":{"rel":["説","说"],"isEdu_HK":true,"isEdu_TW_1":true,"isTrad":true},
"説":{"rel":["說","说"],"isEdu_HK":true,"isTrad":true},
"说":{"rel":["說","説"],"isEdu_CN_1c":true,"isSimp":true},

"裏":{"rel":["裡","里"],"isEdu_HK":true,"isTrad":true},
"裡":{"rel":["裏","里"],"isEdu_HK":true,"isEdu_TW_1":true,"isTrad":true},
"里":{"rel":["裏","裡"],"isEdu_CN_1c":true,"isEdu_HK":true,"isEdu_TW_1":true,"isSimp":true,"isTrad":true},

"床":{"rel":["牀"],"isEdu_CN_1c":true,"isEdu_TW_1":true,"isSimp":true,"isTrad":true},
"牀":{"rel":["床"],"isEdu_HK":true,"isTrad":true},
```
