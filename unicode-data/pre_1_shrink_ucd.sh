...
grep -v -E '^ .*=""'  /tmp/ucd.repertoire.xml | grep  -E "(^\S)|(^ (age)|(na)|(blk)|(UIdeo)|(EqUIdeo)=)|(^ k)"  > /tmp/ucd.repertoire.xml.2
sed -i "s/^ k/  k/g" /tmp/ucd.repertoire.xml.2
grep  -E "(^\S)|(^ \S)|(^  ((k.*Variant=)|(kIICore)|(kIRG_)|(kFrequency)|(kStrange)))"  /tmp/ucd.repertoire.xml.2 > /tmp/ucd.repertoire.xml.3
